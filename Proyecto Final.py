...
import argparse
from fuzzywuzzy import fuzz #Comparar dos string y devuelve un valor de similitud
...
columns_1= ["latitud_df1", "longitud_df1", "parada_cercana", "activate", "disponibilidad", "free_bases", "latitud_df2","longitud_df2", "fuzzy_df1", "fuzzy_df2","ocupación" ]

df_final= df_universidadxbicimad.drop(columns=columns_1)
...
# Agregar marcadores para las universidades
for _, universidad in df_universidadxbicimad.iterrows():
    folium.Marker(
        location=[universidad['latitud_df1'], universidad['longitud_df1']],
        popup=universidad['nombre'],
        icon=folium.Icon(color='blue', icon='university')
    ).add_to(mapa)

# Agregar marcadores para las estaciones de BiciMAD más cercanas
for _, estacion in df_universidadxbicimad.iterrows():
    folium.Marker(
        location=[estacion['latitud_df2'], estacion['longitud_df2']],
        popup=estacion['localizacion'],
